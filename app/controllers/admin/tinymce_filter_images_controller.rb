...
class Admin::TinymceFilterImagesController < ApplicationController
  def upload
    if request.post?
      file_data = params[:file]
      @url = "/images/tinymce_filter/#{Time.now.to_i.to_s}_#{rand(1000000)}.jpg"
      out_path = "public#{@url}"
      ...
      ImageScience.with_image(path) { |img| img.forced_thumbnail(500, ImageScience::WAY_WIDTH) { |thumb| thumb.save out_path } }

      tf.close! unless tf.nil?

      render :action => 'upload_complete', :layout => false
    else
